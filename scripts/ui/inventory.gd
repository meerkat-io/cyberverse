	...
	for component_data in components:
		...
		if packed_scene:
			...
			if component_instance is Component:
				add_component(component_instance, component_data.grid_pos)
			else:
...
func add_component(component: Component, grid_pos: Vector2i) -> bool:
	if not can_place(component, grid_pos):
...
func _ready() -> void:
	...
	var components: Array[Component.ComponentData] = [
		Component.ComponentData.new("res://scenes/component/energy_cell/level_1.tscn", Vector2i(0, 0)),
		Component.ComponentData.new("res://scenes/component/engine/level_1.tscn", Vector2i(2, 0)),
		Component.ComponentData.new("res://scenes/component/intelligent_module/level_1.tscn", Vector2i(4, 0))
	]
...
func _on_component_drag_started(component: Component) -> void:
...
func _place_component(component: Component, grid_pos: Vector2i) -> void:
	if component.get_parent() != container:
		...
		# Connect signals only once to avoid duplicate handling
		if not component.drag_started.is_connected(_on_component_drag_started):
			component.drag_started.connect(_on_component_drag_started.bind(component))
		if not component.drag_ended.is_connected(_on_component_drag_ended):
			component.drag_ended.connect(_on_component_drag_ended.bind(component))
